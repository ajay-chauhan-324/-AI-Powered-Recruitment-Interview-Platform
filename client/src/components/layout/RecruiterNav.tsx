...
import { Briefcase, Calendar, ChevronDown, LayoutDashboard, LogOut, Settings, Sparkles, Users } from 'lucide-react'
...
const NAV_ITEMS = [
  ...
  { to: '/recruiter/ai', label: 'AI Insights', Icon: Sparkles },
]

/** Top nav for the recruiter portal — a separate persona shell from UserNav (candidate) and
 * AdminNav (the original single-tenant admin system), consistent with this codebase's
 * established pattern of one nav component per persona rather than one parameterized nav. */
  ...
  return (
    <header className="relative flex h-14 shrink-0 items-center justify-between gap-4 border-b border-hairline bg-paper-50 px-4 sm:px-6">
      <div className="flex min-w-0 items-center gap-6 overflow-x-auto">
        <Link to="/recruiter/dashboard" className="shrink-0 font-mono text-sm font-medium tracking-wide text-ink-900">
          The Ledger <span className="text-ink-500">· Recruiter</span>
        </Link>
        <nav className="flex items-center gap-0.5 text-sm text-ink-700">
          {NAV_ITEMS.map((item) => {
